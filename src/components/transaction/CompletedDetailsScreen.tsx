...
const CompletedDetailsScreen: React.FC<CompletedDetailsProps> = ({
  booking,
  setCurrentStep,
}) => {
  ...
  const formattedAddress = `${technician?.buildingName || ""}, ${
    technician?.areaName || ""
    ...
    <View className="flex-1 bg-gray-50">
      ...
      >
        ...
        <View className="bg-white rounded-2xl p-5 mb-6 shadow-md">
          <DetailRow
            icon={<User width={20} height={20} color="#2563EB" />}
            label="Technician"
            value={technician?.username}
          />
          <DetailRow
            icon={<Star width={20} height={20} color="#F97316" />}
            label="Service"
            value={service?.serviceName}
          />
          <DetailRow
            icon={<Phone width={20} height={20} color="#16A34A" />}
            label="Contact"
            value={technician?.phoneNumber}
          />
          <DetailRow
            icon={<MapPin width={20} height={20} color="#DC2626" />}
            label="Address"
            value={formattedAddress}
            multiline
          />
          <DetailRow
            icon={<Text className="text-purple-700 text-xl font-bold">₹</Text>}
            label="Total Price"
...
const DetailRow: React.FC<DetailRowProps> = ({ icon, label, value, multiline }) => (
