...
const UpcomingDetailsScreen: React.FC<UpcomingDetailsProps> = ({
  booking,
  setCurrentStep,
  setActiveTab,
  refetchBookings,
  ...
  const formattedAddress = `${technician?.buildingName || ""}, ${technician?.areaName || ""}, ${technician?.city || ""}, ${technician?.state || ""} - ${technician?.pincode || ""}`.trim();
  ...
  const handleConfirmCancel = async () => {
    ...
    try {
      ...
      const payload = { orderId: bookingState._id, userId };
      const response = await bookingCancleByUser(payload);

      if (response?.success) {
        Alert.alert("Booking Cancelled", "Your booking was cancelled successfully.");
    ...
    <View className="flex-1 bg-gray-50">
      ...
      >
        ...
        <View className="bg-white rounded-2xl p-5 mb-6 shadow-md">
          <DetailRow icon={<User width={20} height={20} color="#2563EB" />} label="Technician" value={technician?.username} />
          <DetailRow icon={<Globe width={20} height={20} color="#F97316" />} label="Service" value={service?.serviceName} />
          <DetailRow icon={<Phone width={20} height={20} color="#16A34A" />} label="Contact" value={technician?.phoneNumber} />
          <DetailRow icon={<MapPin width={20} height={20} color="#DC2626" />} label="Address" value={formattedAddress} multiline />
          <DetailRow icon={<Text className="text-purple-700 text-xl font-bold">₹</Text>} label="Total Price" value={`₹${bookingState.totalPrice}`} />
          <DetailRow icon={<Key width={20} height={20} color="#FBBF24" />} label="OTP" value={bookingState.otp.toString()} badge />
        </View>
        ...
        {bookingState.status === "upcoming" ? (
          <TouchableOpacity
            ...
            disabled={isCancelling}
          >
            <Text className="text-black font-semibold text-center text-base">Cancel Service</Text>
          </TouchableOpacity>
        ) : (
...
/** Sub-component for consistent info row */
...
const DetailRow: React.FC<DetailRowProps> = ({ icon, label, value, multiline, badge }) => (
  <View className="flex-row items-start gap-4 mb-4">
    <View className="w-10 h-10 bg-gray-100 rounded-full items-center justify-center">{icon}</View>
